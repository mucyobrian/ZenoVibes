...
const DB = (() => {
  ...
  function normalizeProduct(p) {
    ...
    return {
      ...
      listingType: p.listingtype || p.listing_type || (parseFloat(String(p.price||'0').replace(/[^0-9.]/g,'')) === 0 ? 'free' : 'sell'),
      images,
  ...
  async function saveMyListing(product) {
    ...
    try {
      ...
      const payload = {
        ...
        listingType:    newProduct.listingType || 'sell',
  ...
  // Counts only PAID (non-free) listings posted in the current calendar month.
  // Free giveaways don't count against the monthly posting limit, since they're
  // not for profit.
  async function getMonthlyPaidCount() {
    const myList = await getMyListings();
    const now = new Date();
    return myList.filter(p => {
      const d = new Date(p.timestamp);
      const sameMonth = d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth();
      return sameMonth && p.listingType !== 'free';
    }).length;
  }
  ...
  async function incrementMonthlyCount() {
    ...
    // Free listings are excluded — they're giveaways, not "for profit" posts.
    const count = await getMonthlyPaidCount();
    if (count >= limit) {
  ...
  // ── Pesapal payment ──────────────────────────────
  // Asks the Apps Script backend to open a Pesapal order for this user's
  // plan upgrade. Backend talks to Pesapal directly (consumer_key/secret
  // never touch the browser) and hands back a redirect_url to send the
  // user to. Throws on failure so callers can show an error toast.
  async function createPaymentOrder() {
    const user = getCurrentUser();
    if (!user) throw new Error('You need to be signed in to upgrade.');
    ...
    const res = await fetch(CONFIG.LISTINGS_API_URL, {
      ...
      body: JSON.stringify({
        ...
        phone: user.phone,
        name: user.name,
        amount: CONFIG.PAID_PRICE,
        currency: CONFIG.PAID_CURRENCY,
        // Pesapal redirects the browser here after checkout. Always send
        // an absolute URL — this works whether called from account.html
        // or anywhere else under /pages/.
        callbackUrl: new URL('payment-callback.html', window.location.href).toString(),
      }),
    });
    const result = await res.json();
    ...
    return result; // { success, redirectUrl, orderTrackingId, merchantReference }
  }

  // Polls the backend for the status of a previously created order.
  // Used by payment-callback.html after Pesapal redirects the user back.
  async function checkPaymentStatus(orderTrackingId) {
    const res = await fetch(CONFIG.LISTINGS_API_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'text/plain' },
      body: JSON.stringify({ action: 'checkPaymentStatus', orderTrackingId }),
    });
    const result = await res.json();
    if (!result.success) throw new Error(result.error || 'Could not check payment status.');
    return result; // { success, status: 'COMPLETED'|'PENDING'|'FAILED', plan }
  }
