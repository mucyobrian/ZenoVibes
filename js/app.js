...
// Shared by index.html (home.js) and pages/categories.html.
const SUBCAT_EMOJI = {
...
// Looks up the subcategory's own thumbnail from js/subcat-images.js.
// Falls back to the parent category's image if SUBCAT_IMAGES isn't
// loaded yet or doesn't have an entry for this category:subcategory pair.
function getSubcatImage(catId, subcat) {
  const key = `${catId}:${subcat}`;
  if (typeof SUBCAT_IMAGES !== 'undefined' && SUBCAT_IMAGES[key]) {
    return SUBCAT_IMAGES[key];
  }
  const cat = CONFIG.CATEGORIES.find(c => c.id === catId);
...
function buildProductCard(product, userLat, userLng, listView) {
  ...
  // Engagement badge (stars + views) from cache if available
  const eng = (typeof engagementCache !== 'undefined') ? engagementCache[product.id] : null;
  const engBadge = eng && (eng.avgRating > 0 || eng.views > 0) ? `
    <div class="card-eng-badge">
      ${eng.avgRating > 0 ? `<span class="card-stars">${'★'.repeat(Math.round(eng.avgRating))}${'☆'.repeat(5-Math.round(eng.avgRating))}</span><span class="card-avg">${parseFloat(eng.avgRating).toFixed(1)}</span>` : ''}
      ${eng.views > 0 ? `<span class="card-views">👁 ${eng.views >= 1000 ? (eng.views/1000).toFixed(1)+'k' : eng.views}</span>` : ''}
    </div>` : '';
